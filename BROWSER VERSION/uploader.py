...
LOCAL_VERSION_DIR = os.path.join(PROJECT_DIR, "LOCAL VERSION")
LOCAL_OUT_DIR = os.path.join(LOCAL_VERSION_DIR, "output")
FALLBACK_OUTPUT_DIR = os.path.join(PROJECT_DIR, "output")
...
TIKTOK_UPLOADS_FILE = os.path.join(LOCAL_OUT_DIR, "tiktok_uploads.json")
TIKTOK_UPLOADS_FILE_FALLBACK = os.path.join(FALLBACK_OUTPUT_DIR, "tiktok_uploads.json")
...
def find_all_videos():
    """Encuentra todos los videos con sus metadatos, priorizando LOCAL VERSION"""
    videos = []
    seen_files = set()

    # Priorizar LOCAL VERSION/output, luego fallback output
    for out_dir in [LOCAL_OUT_DIR, FALLBACK_OUTPUT_DIR]:
        ...
        for json_file in sorted(glob.glob(os.path.join(out_dir, "*_video_info.json"))):
            try:
                ...
                source = "LOCAL" if "LOCAL VERSION" in out_dir else "ROOT"
...
def _get_tiktok_uploads_path():
    ...
    return TIKTOK_UPLOADS_FILE_FALLBACK
